...
using StudentManagement.BLL.Mapping.Profiles;
...
namespace StudentManagement.Tests.Unit.Controller
{
    public class StudentControllerTests
    {
        ...
        public StudentControllerTests()
        {
            ...
            var _classRepository = new ClassRepositry(_context);
            var _courseRepository = new CourseRepository(_context);
            var _classStudentRepository = new ClassStudentRepository(_context);
            var _registerCancellationRepository = new RegisterCancellationHistoryRepository(_context);
            ...
            {
                cfg.AddMaps(typeof(StudentProfile).Assembly); // Hoặc typeof(MappingProfile).Assembly
            });
            ...
            var _classService = new ClassService(
                _classRepository, 
                _courseRepository, 
                _classStudentRepository, 
                _registerCancellationRepository,
                _mapper);

            var _classStudentService = new ClassStudentService(
                _classStudentRepository,
                _studentRepository,
                _classRepository,
                _registerCancellationRepository,
            ...
            _studentsController = new StudentsController(_studentService, _classService, _classStudentService);
        }
        ...
        public async Task UpdateStudentAsync_StatusNotExists_ShouldReturnUpdatedStudent()
        {
            ...
            Assert.Null(data);
            Assert.Equal("INVALID_STATUS", response.Error?.Code);
        }
